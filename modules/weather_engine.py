...
class WeatherEngine:
    ...
    def get_weather_grid(
        ...
        try:
            ...
            # Initialize FastMeteo Grid with local cache
            grid = Grid(local_store=self.cache_dir)
            
            # Convert flight level to altitude in meters
            # Flight level is in hundreds of feet: FL350 = 35,000 ft
            altitude_ft = flight_level * 100
            altitude_m = altitude_ft * 0.3048
            
            logger.info(f"  Altitude: {altitude_ft:.0f} ft ({altitude_m:.0f} m)")
            
            # Fetch weather data along the flight path
            # FastMeteo's interpolate_flight automatically handles the route
            logger.info("Fetching weather data from FastMeteo...")
            
            weather_data = grid.interpolate_flight(
                origin=(origin_lat, origin_lon),
                destination=(dest_lat, dest_lon),
                flight_level=flight_level,
                date=flight_datetime
            )
            ...
            # Format data for OpenAP compatibility
            formatted_data = self._format_for_openap(weather_data, altitude_m, flight_datetime)
    ...
    def _format_for_openap(
        ...
        try:
            ...
            # Column mapping: FastMeteo → OpenAP
            column_mapping = {
                'u_component_of_wind': 'u',
                'v_component_of_wind': 'v',
                'altitude': 'h',
            ...
            # Add altitude if not present
            if 'h' not in df.columns:
                df['h'] = altitude_m
                logger.debug(f"Added altitude column: {altitude_m:.0f} m")
            
            # Add temperature if not present (use ISA standard atmosphere approximation)
            if 't' not in df.columns:
                # ISA standard: T = 288.15 - 0.0065 * h (for troposphere)
                df['t'] = 288.15 - 0.0065 * df['h']
                logger.debug("Added temperature column using ISA approximation")
            
            # Handle timestamp
            if 'ts' not in df.columns:
                # Create relative timestamps starting from 0
                if len(df) > 1:
                    # Assume even spacing over typical flight duration
                    total_points = len(df)
                    # Estimate flight duration based on distance and typical cruise speed
                    if 'lat' in df.columns and 'lon' in df.columns:
                        ...
                        # Typical cruise speed: 450 knots ≈ 833 km/h
                        flight_duration_hours = distance_km / 833
                        flight_duration_seconds = flight_duration_hours * 3600
                        
                        # Create evenly spaced timestamps
                        df['ts'] = np.linspace(0, flight_duration_seconds, total_points)
                    else:
                        # Fallback: 1-minute intervals
                        df['ts'] = np.arange(0, len(df) * 60, 60)
                else:
                    df['ts'] = 0
                
                logger.debug(f"Created timestamp column: {df['ts'].min():.0f} to {df['ts'].max():.0f} seconds")
            else:
                # Convert existing timestamp to relative seconds if needed
                if pd.api.types.is_datetime64_any_dtype(df['ts']):
                    start_time = df['ts'].min()
                    df['ts'] = (df['ts'] - start_time).dt.total_seconds()
                    logger.debug("Converted timestamp to relative seconds")
            ...
            # Convert wind components to m/s if they're in other units
            # FastMeteo typically returns m/s, but verify reasonable values
            if 'u' in df.columns and 'v' in df.columns:
                u_mean = df['u'].abs().mean()
                v_mean = df['v'].abs().mean()
                
                # If values are very large (>100), they might be in different units
                if u_mean > 100 or v_mean > 100:
                    logger.warning(f"Wind values seem large (u: {u_mean:.1f}, v: {v_mean:.1f}), assuming conversion needed")
                    # Could be km/h, convert to m/s
                    df['u'] = df['u'] / 3.6
                    df['v'] = df['v'] / 3.6
                else:
                    logger.debug(f"Wind speeds: u={u_mean:.1f} m/s, v={v_mean:.1f} m/s")
...
def main():
    ...
    weather_grid = engine.get_weather_grid(
        origin=(19.0896, 72.8656),  # Mumbai (BOM)
        dest=(9.9312, 76.2673),      # Kochi (COK)
        flight_date='2025-01-08 14:45:00',
        flight_level=350
    )
